...
func testNewWithIsRegistered(t *testing.T) {
	...
	l.MonitorEvent(monitor.Event{Err: errors.New("service discovery error")})
	l.MonitorEvent(monitor.Event{Stopped: true})
	l.MonitorEvent(monitor.Event{})
	l.MonitorEvent(monitor.Event{Instances: []string{"keep", "disconnect"}})
